...
namespace aspect
{
  namespace Postprocess
  {
    ...
    MeltConveyFour<dim>::execute (TableHandler &statistics)
    {
        ...
        if (cell->is_locally_owned())
          {
			 ...
			 for (unsigned int q = 0; q < n_q_points; q++)
			{
				...
				target_mantle_melting_max = std::max(target_mantle_melting_max, std::abs(target_mantle_melting_values[q]));
				area_in_km += fe_values.JxW(q) / 1e6;
				fraction_max = std::max(fraction_max, std::abs(melt_fraction_values[q]));
				//20250217
				mantle_lower_average += std::max(0., mantle_lower_values[q] * fe_values.JxW(q) / 1e6);
				
				
			} 
								
			//地幔熔融传输条件4：岩石圈地幔与沉积的边界区域
			//暂时规定两种熔融产生的物质不能混在一起
			...
			const bool mantle_convey_start_6 = upper_average / area_in_km < 0.02
											 ...
											 //20250217
											 //&& target_mantle_melting_max < 0.9
											 && target_crustal_melting_average / area_in_km < 0.02;
											 
			//20250217 
			//地幔熔融传输条件7：新生地壳与下岩石圈地幔的边界区域（测试）
			//主要针对亏损地幔上涌到地表的情况
			const bool mantle_convey_start_7 = upper_average / area_in_km < 0.02
											 && lower_average / area_in_km < 0.02
											 ...
											 && target_mantle_melting_average / area_in_km < 0.5
											 && target_crustal_melting_average / area_in_km < 0.02;
											 
			//地壳熔融传输条件4：暂无
			//1008 早期地球，地壳熔融产物边缘位置
			//可替代地幔熔融产物
			const bool crustal_convey_start_4 = target_crustal_melting_average / area_in_km > 0.02
			...
			if (include_mantle_melting 
			     && (mantle_convey_start_4 || mantle_convey_start_5 || mantle_convey_start_6 || mantle_convey_start_7) 
				 && fraction_max < 1e-2
      ...
      statistics.add_value ("Melt Convey 4", 0);
	  {
        const char *columns[] = {"Melt Convey 4"};
	...
	template <int dim>
    void
    MeltConveyFour<dim>::parse_parameters (ParameterHandler &prm)
	{
...
namespace aspect
{
  namespace Postprocess
  {
    ASPECT_REGISTER_POSTPROCESSOR(MeltConveyFour,
                                  "melt convey 4",
