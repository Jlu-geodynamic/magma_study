...
#include </fs2/home/liuzhonglan/wy/lib_extra/melt20241204/grain/include/postprocess/melt_convey_3.h>
...
namespace aspect
{
  namespace Postprocess
  {
    ...
    MeltConveyThree<dim>::execute (TableHandler &statistics)
    {
        ...
        if (cell->is_locally_owned())
          {
			...
			//地幔熔融传输条件3：新生地壳与岩石圈地幔的边界区域
			//暂时规定两种熔融产生的物质不能混在一起
			const bool mantle_convey_start_3 = upper_average / area_in_km < 0.02
			...
			//地壳熔融传输条件3：暂无
			//1008 早期地球，仅规定地壳熔融产物出现的深度
			const bool crustal_convey_start_3 = target_crustal_melting_average / area_in_km < 0.02
			                                 &&  max_y - posy <= 30e3;
			//考虑到aspect规定通道0为master读写通道，为防止输出结果时出现读写冲突，当mpi通道为0时不占用通道向容器内读写
			//即模型的最左侧（根据精度情况，约为最左侧10~15km）不启用熔体运输功能
			if (include_mantle_melting 
			     && mantle_convey_start_3 
				 && fraction_max < 1e-2
      ...
      statistics.add_value ("Melt Convey 3", 0);
	  {
        const char *columns[] = {"Melt Convey 3"};
	...
	template <int dim>
    void
    MeltConveyThree<dim>::parse_parameters (ParameterHandler &prm)
	{
...
namespace aspect
{
  namespace Postprocess
  {
    ASPECT_REGISTER_POSTPROCESSOR(MeltConveyThree,
                                  "melt convey 3",
