...
namespace aspect
{
  namespace InitialComposition
  {
    ...
    Rift<dim>::initialize ()
    {
      AssertThrow(dynamic_cast<const MaterialModel::ViscoPlasticGrainSize<dim> *>(&this->get_material_model()) != NULL,
    ...
    Rift<dim>::parse_parameters (ParameterHandler &prm)
    {
	  ...
	  else if(this->introspection().compositional_name_exists("viscous_strain_grainsize"))
	              strain_composition_number = this->introspection().compositional_index_for_name("viscous_strain_grainsize");	  
      else
          AssertThrow(false, ExcMessage("This plugin requires a compositional strain field (plastic_strain, viscous_strain, viscous_strain_grainsize or total_strain). "));
