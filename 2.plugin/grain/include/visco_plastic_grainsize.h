...
// 原文件位置以及名称：material_model/visco_plastic.h
//                     material_model/rheology/visco_plastic.h
//					   material_model/rheology/diffusion_creep.h
//					   material_model/grain_size.h
...
namespace aspect
{
  namespace MaterialModel
  {
     ...
     */
    template <int dim>
    class ViscoPlasticGrainSize : public MaterialModel::Interface<dim>, public ::aspect::SimulatorAccess<dim>
    {
      public:
		...
		std::array<double, 6> melted_judgement(const double temperature,
      ...
      private:
         ...
         */
        std::unique_ptr<Rheology::ViscoPlasticGrainSize<dim>> rheology;
		...
		unsigned int min_crood_y;
		std::string path_mantle_melting;
		std::string path_crustal_melting;
		...
		std::vector<double> init_grain_size;
		...
		bool include_hydrous_melting;
		double D_H2O;
		double K;
		double gamma;
		
		bool include_carbonate_melting;
		double X_CO2;
		...
		double B3_crust;
		double gamma_crust;
		double crustal_melt_temp_reduction;
